...
var AudioManager = {
    ...
    playChestAudio: function(url, loop) {
        this.stopAll();  // Сначала выключаем всё, что играло
        
        if (!url || url.trim() === "") return;  // Если ссылка пустая — выходим
        
        try {
            var audio = new Audio(url);  // Создаём новый аудиоэлемент
            audio.loop = loop || false;  // Зацикливаем, если нужно
            audio.volume = state.chestAudioVolume * state.globalVolume;  // Громкость
            audio.play().catch(function(e) {
                // Браузер может заблокировать автовоспроизведение — это нормально
            });
            
            if (!loop) {
                // Обычное аудио (не зацикленное) — сохраняем ссылку и удаляем при завершении
                ...
                // Зацикленное аудио (режим «Задолбал») — сохраняем в отдельную переменную
                state.currentAnnoyingAudio = audio;
            }
        } catch (e) {
            // Если что-то пошло не так — молча игнорируем
        }
    },
